...
helpers.discernParser = function (fileName, opts_) {
  ...
  if (typeof parser === 'undefined') {
    console.log(chalk.gray('[indian-ocean]') + ' ' + chalk.cyan('Notice:'), '"' + chalk.bold(format) + '" extension not supported on file: "' + chalk.bold(fileName) + '". Parsing as plain text...')
    parser = parsers['txt']
  }
  return parser
}
...
helpers.discernFileFormatter = function (fileName) {
  ...
  if (typeof formatter === 'undefined') {
    console.log(chalk.gray('[indian-ocean]') + ' ' + chalk.cyan('Notice:'), '"' + chalk.bold(format) + '" extension not supported on file: "' + chalk.bold(fileName) + '". Writing out as plain text...')
 ...
 * @param {String} fileName the name of the file
 * @param {Object} [options] optional See options below
 * @param {Boolean} [options.flatten] optional If true, input JSON data with nested objects will be flattened.
 * @param {String|Function|Object} [options.parser] optional This can be a string that is the file's delimiter or a function that returns the json. See `parsers` in library source for examples. For convenience, this can also take a dsv object such as `dsv.dsv('_')` or any object that has a `parse` method. Pre-version 1.1 this was called `parseWith` and to maintain support, that will still work.
...
shorthandReaders.readJson = function (path, cb) {
  readers.readData(path, {parser: parsers.json, flatten: true}, cb)
}
...
shorthandReaders.readJsonSync = function (path) {
  return readers.readDataSync(path, {parser: parsers.json, flatten: true})
}
...
module.exports = _.extend({}, readers, shorthandReaders, writers, helpers, { fs: fs })
