 ...
 *
 * @param {String} fileName the name of the file
 * @param {Object} options optional `parseWith` value to use when reading the file. This can be a string that is the file's delimiter or a parsing object similar to what `dsv.dsv` returns. Parsing objects have a `parse` method that returns the parsed data.
 ...
 * @param {Boolean} [options.includeMatchAll=false] If true, require all include conditions to be met for a file to be included. Default is `false`.
 * @param {Boolean} [options.excludeMatchAll=false] If true, require all exclude conditions to be met for a file to be excluded. Default is `false`.
 * @param {Boolean} [options.fullPath=false] If `true` the full path of the file, otherwise return just the file name.
 * @param {Boolean} [options.skipFiles=false] If `true`, only include directories. Default is `false`.
 * @param {Boolean} [options.skipDirectories=false] If `true`, only include files. Default is `false`.
 ...
 * })
 *
 * io.writeDbfToData('path/to/create/to/data.csv', dbfObj, {makeDirectories: true}, function(err){
 *   console.log(err)
 * })
 */
