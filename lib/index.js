  ...
  function filter (async) {
    return function (err, files) {
      ...
      var filtered = files.filter(function (fileName) {
        return _.all(inOrExcludesArr, function (includeExtension) { return _.isEqual(modeInfo.include, helpers.matches(fileName, includeExtension)) })
      })
 ...
 *
 * @example
 * // dir contains `data-0.tsv`, `data-0.json`, `data-0.csv`, `data-1.csv`
 ...
 * console.log(files) // ['data-0.csv', 'data-1.csv']
 *
 * var files = io.readdirIncludeSync('path/to/files', ['csv', 'tsv'], true)
 * console.log(files) // ['path/to/files/data-0.csv', 'path/to/files/data-1.csv', 'path/to/files/data-0.tsv']
 *
 */
 ...
 *   console.log(files) // ['data-0.tsv', 'data-0.json']
 * })
 *
