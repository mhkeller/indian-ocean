...
var parsers = {
  geocsv: csv2geojson.csv2geojson,
  json: JSON,
  csv: dsv.csv,
  tsv: dsv.tsv,
  psv: dsv.dsv('|')
 ...
 *
 * @example
 * tk.discernFormat('path/to/data.csv') -> 'csv'
 */
 ...
 *
 * @example
 * var parser = tk.discernParser('path/to/data.csv');
...
/**
 * Asynchronously read a comma-separated value file. If the file contains columns headers that indicate points (eg, "lat" and "lng"), a GeoJSON object will be returned.
 *
 * @param {String} fileName the name of the file
 * @param {Function} callback callback used when read data is read, takes error (if any) and the data read
 */
readers.readCsv = function (path, cb) {
  fs.readFile(path, 'utf8', function (err, data) {
    if (err) {
      ...
      parsers.geocsv(data, function (err, geojsonData) {
        if (err) {
          cb(null, parsers.csv.parse(data))
        } else {
          cb(null, geojsonData)
        }
      })
