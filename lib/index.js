...
var formatters = {
  json: function (file, writeOptions) {
    ...
    return JSON.stringify(file, writeOptions.replacer, writeOptions.space)
  },
  csv: function (file, writeOptions) {
    file = formattingPreflight(file, 'csv')
    try {
      return dsv.csvFormat(file, writeOptions)
    } catch (err) {
  ...
  tsv: function (file, writeOptions) {
    ...
    try {
      return dsv.tsvFormat(file, writeOptions)
    } catch (err) {
  ...
  psv: function (file, writeOptions) {
    ...
    try {
      return dsv.dsvFormat('|').format(file, writeOptions)
    } catch (err) {
...
var parsers = {
  json: function (str, readOptions) {
    readOptions = readOptions || {}
    return jsonParser(str, readOptions.transform, readOptions.filename)
  },
  csv: function (str, readOptions) {
    readOptions = readOptions || {}
    return dsv.csvParse(str, readOptions.transform)
  },
  tsv: function (str, readOptions) {
    readOptions = readOptions || {}
    return dsv.tsvParse(str, readOptions.transform)
  },
  psv: function (str, readOptions) {
    readOptions = readOptions || {}
    return dsv.dsvFormat('|').parse(str, readOptions.transform)
  },
  txt: function (str, readOptions) {
    return readOptions && _.isFunction(readOptions.transform) ? readOptions.transform(str) : str
  },
  yaml: yamlParser.load,
  aml: function (str, readOptions) {
    readOptions = readOptions || {}
    var transform = readOptions.transform || function (d) { return d }
    delete readOptions.transform
    return transform(archieml.load(str, readOptions))
  }
}
 ...
 *
 * @param {String} fileName the name of the file
 * @param {Object|Function} [options] Optional. Set this as a function as a shorthand for `transform`.
 ...
 * @param {Function} [options.transform] Transformation function. See {@link shorthandReaders} for format-specific behavior.
 ...
 * })
 *
 * // Shorthand for specifying a transform function
 ...
 * })
 *
 * // Explicitly specify a transform function and a filename for a json file. See `readJson` for more details
...
readers.readData = function (path, opts_, cb_) {
  ...
  if (arguments.length === 3) {
    ...
    if (opts_ && opts_.readOptions) {
      if (_.isFunction(opts_.readOptions)) {
        readOptions = {transform: opts_.readOptions}
      } else {
        readOptions = opts_.readOptions
      }
    } else if (opts_) {
      if (_.isFunction(opts_)) {
        readOptions = {transform: opts_}
      } else {
        readOptions = opts_
      }
    }
 ...
 *
 * @param {String} fileName the name of the file
 * @param {Function} [transform] Optional transformation function, called once for each row (header row skipped). Has signature `(row, i, columns)`. See example below or d3-dsv documentation for details.
 * @param {Function} callback callback used when read data is read, takes error (if any) and the data read
 *
 * @example
...
shorthandReaders.readCsv = function (path, opts_, cb) {
  ...
  if (typeof cb === 'undefined') {
    ...
    readOptions = _.isFunction(opts_) ? {transform: opts_} : opts_
  }
 ...
 *
 * @param {String} fileName the name of the file
 * @param {Function|Object} [transform] Optional transformation function, called once for each row (header row skipped). Has signature `(row, i, columns)`. See example below or d3-dsv documentation for details.
 ...
 *
 * @param {String} fileName the name of the file
 * @param {Function|String|Object} [readOptions] Can be a transformation function, a filename string that's display as an error message or an object specifying both.
 * @param {Function} [readOptions.transform] Prescribes how the value originally produced by parsing is transformed, before being returned. See JSON.parse docs for more: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse#Using_the_reviver_parameter
 * @param {String} [readOptions.filename] Filename displayed in the error message.
 ...
 *
 * // Specify a transform
 * io.readJson('path/to/data.json', function (k, v) {
 *   if (typeof v === 'number') {
 *     return v * 2
 *   }
 *   return v
 ...
 * })
 *
 * // Specify a transform and a filename
...
shorthandReaders.readJson = function (path, opts_, cb) {
  ...
  readers.readData(path, {parser: parsers.json, readOptions: readOptions}, cb)
}
...
shorthandReaders.readJsonSync = function (path, opts_) {
  ...
  return readers.readDataSync(path, {parser: parsers.json, readOptions: readOptions})
}
 ...
 *
 * @param {String} fileName the name of the file
 * @param {Function} [transform] Optional transformation function, take the file and returns any transformed value
 * @param {Function} callback callback used when read data is read, takes error (if any) and the data read
 *
 * @example
 ...
 *
 * @param {String} fileName the name of the file.
 * @param {Object} [options] Settings to be passed to `archieml` library. Check here for latest supported options: https://www.npmjs.com/package/archieml
 * @param {Boolean} [options.comments=false] Allow comments.
 * @param {Function} callback callback used when read data is read, takes error (if any) and the data read.
 *
 * @example
 ...
 * })
 *
 * // With ArchieML comments allowed
 * io.readAml('path/to/data-with-comments.aml', {comments: true}, function (err, data) {
 *   console.log(data) // json data
 * })
 */
 ...
 * console.log(data) // json data
 *
 * var data = io.readAmlSync('path/to/data-with-comments.aml', {comments: true})
 * console.log(data) // json data
 */
shorthandReaders.readAmlSync = function (path, opts_) {
  ...
  return readers.readDataSync(path, {parser: parsers.aml, readOptions: readOptions})
}
