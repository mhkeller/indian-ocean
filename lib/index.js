 ...
 * @param {String} fileName the name of the file
 * @returns {Boolean} whether there was an error, indicating the existence of the file or not
 *
 * @example
 * var exists = io.exists('path/to/data.tsv', function(err){
 *   console.log(err) // If error is `true` then file does not exist. If `false` it exists 
 * });
 * 
 */
helpers.exists = function (filename, cb) {
 fs.access(function(err) { cb(err ? true : false); });
}
 ...
 *
 * @param {String} fileName the name of the file
 * @param {String} whether the file exists or not
